...
course_diversity <- function(sr,sc)
{
  tab <- all_diversity(sr,sc)
  tab <- tab %>% select(-c('TERM_SHORT_DES'))
  sc  <- sc %>% left_join(tab,by=c('TERM_CD','SBJCT_CD','CATLG_NBR','CLASS_NBR'))
  
  sum <- sc %>% group_by(STDNT_ID) %>% 
                summarize(MEAN_DEMO_DIV=sum(DEMO_DIV*UNITS_ERND_NBR,na.rm=TRUE)/sum(UNITS_ERND_NBR,na.rm=TRUE),
                          MEAN_MAJOR_DIV=sum(MAJOR_DIV*UNITS_ERND_NBR,na.rm=TRUE)/sum(UNITS_ERND_NBR,na.rm=TRUE))         
